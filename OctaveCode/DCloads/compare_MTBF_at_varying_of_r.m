...
% Component Redundancy (CR)
  ...
  [A_SR(counter),MTTF_SR,MTBF_SR(counter)] = evaluate_SR(n,lambdaB_SR,lambdaR_SR,lambdaU,lambdaD,muB_SR,muR_SR,muU);
  [A_MR(counter),MTTF_MR,MTBF_MR(counter)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaU,lambdaD_MR,muB_MR,muR_MR,muU);
  [A_IR(counter),MTTF_IR,MTBF_IR(counter)] = evaluate_IR(nm,n,lambdaB_IR,lambdaR_IR,lambdaU,lambdaD,muB_IR,muR_IR,muU);
  [A_CR(counter),MTTF_CR,MTBF_CR(counter)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaU,lambdaD,muB_CR,muR_CR,muU);
  %
...
csvwrite('results-r17-muU2-lambdaD0.125-varyingR.csv', [r I_SR I_IR I_CR]);
csvwrite('results-Availability-r17-muU2-lambdaD0.125-varyingR.csv', [r A_SR A_IR A_CR A_MR]);
