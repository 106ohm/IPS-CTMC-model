...
r_CR=(log(n+emme)+log(2+n+emme))/log(6);
r_IR=(log(3)+log(n+emme))/log(6);
r_MR=(log(2*(n+emme)+1))/log(6);
...
for i = 1 : numberOfEvaluations
  ...
  [A_SR_min(i),MTTF_SR_min,MTBF_SR_min(i)] = evaluate_SR(n,lambdaB_SR,lambdaR_SR,lambdaU,lambdaD(i),muB_SR,muR_SR,muU_min);
  [A_MR_min(i),MTTF_MR_min,MTBF_MR_min(i)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaU,lambdaD_MR(i),muB_MR,muR_MR,muU_min);
  [A_IR_min(i),MTTF_IR_min,MTBF_IR_min(i)] = evaluate_IR(nm,n,lambdaB_IR,lambdaR_IR,lambdaU,lambdaD(i),muB_IR,muR_IR,muU_min);
  [A_CR_min(i),MTTF_CR_min,MTBF_CR_min(i)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaU,lambdaD(i),muB_CR,muR_CR,muU_min);
  %
  ...
  [A_SR_max(i),MTTF_SR_max,MTBF_SR_max(i)] = evaluate_SR(n,lambdaB_SR,lambdaR_SR,lambdaU,lambdaD(i),muB_SR,muR_SR,muU_max);
  [A_MR_max(i),MTTF_MR_max,MTBF_MR_max(i)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaU,lambdaD_MR(i),muB_MR,muR_MR,muU_max);
  [A_IR_max(i),MTTF_IR_max,MTBF_IR_max(i)] = evaluate_IR(nm,n,lambdaB_IR,lambdaR_IR,lambdaU,lambdaD(i),muB_IR,muR_IR,muU_max);
  [A_CR_max(i),MTTF_CR_max,MTBF_CR_max(i)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaU,lambdaD(i),muB_CR,muR_CR,muU_max);
  %
...
csvwrite('results-r17-muU2-varyingLambdaD.csv', [A_U_min A_SR_min MTBF_SR_min A_MR_min MTBF_MR_min A_IR_min MTBF_IR_min A_CR_min MTBF_CR_min A_U_max A_SR_max MTBF_SR_max A_MR_max MTBF_MR_max A_IR_max MTBF_IR_max A_CR_max MTBF_CR_max lambdaD]);
