function [Qre,Qav,pi0,av_indx] = CTMC_SR(n,lambdaB,lambdaR,lambdaI,lambdaU,lambdaD,muB,muR,muU)
...
Rre(1,2) = 2*(lambdaB + lambdaR + lambdaI);
Rre(2,1) = muB;
Rre(2,3) = lambdaB + lambdaR + lambdaI;

% model btteries fault and recovery when the utility is KO
Rre(3+1,3+2) = 2*(lambdaB+lambdaI)+n*lambdaD;
Rre(3+2,3+1) = muB;
Rre(3+2,3+3) = lambdaB+lambdaI+n*lambdaD;
