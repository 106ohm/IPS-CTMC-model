...
for i = 1 : numberOfEvaluations
  ...
  Load(i) = minLoad + i*loadStep; % kW, typical values from 50 to 100 kW

  n = ceil(Load(i)/ModulePower);
  nm = n+3;% nm is n+m
  emme=nm-n; % m
  ...
  [A_MR(i),MTTF_MR,MTBF_MR(i)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaI_MR,lambdaU,lambdaD_MR,muB_MR,muR_MR,muU);
  [A_CR(i),MTTF_CR,MTBF_CR(i)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaI_CR,lambdaU,lambdaD,muB_CR,muR_CR,muU);

end


csvwrite('results-AC-r17-muU2-lambdaD0.125.csv', [Load A_SR MTBF_SR A_MR MTBF_MR A_IR MTBF_IR A_CR MTBF_CR]);
