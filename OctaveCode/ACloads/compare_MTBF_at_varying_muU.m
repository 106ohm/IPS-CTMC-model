...
muU = zeros(numberOfEvaluations,1);
for i = 1 : numberOfEvaluations

  muU(i) = 0.5 + (10-0.5)/numberOfEvaluations*(i-1);

  [A_U_min(i), MTTF_U_min, MTBF_U_min] = evaluate_U(lambdaU,muU(i));
  [A_SR_min(i),MTTF_SR_min,MTBF_SR_min(i)] = evaluate_SR(n,lambdaB_SR,lambdaR_SR,lambdaI_SR,lambdaU,lambdaD_min,muB_SR,muR_SR,muU(i));
  [A_MR_min(i),MTTF_MR_min,MTBF_MR_min(i)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaI_MR,lambdaU,lambdaD_MR_min,muB_MR,muR_MR,muU(i));
  [A_IR_min(i),MTTF_IR_min,MTBF_IR_min(i)] = evaluate_IR(nm,n,lambdaB_IR,lambdaR_IR,lambdaI_IR,lambdaU,lambdaD_min,muB_IR,muR_IR,muU(i));
  [A_CR_min(i),MTTF_CR_min,MTBF_CR_min(i)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaI_CR,lambdaU,lambdaD_min,muB_CR,muR_CR,muU(i));
  %
  ...
  %%%
  [A_U_max(i), MTTF_U_max, MTBF_U_max] = evaluate_U(lambdaU,muU(i));
  [A_SR_max(i),MTTF_SR_max,MTBF_SR_max(i)] = evaluate_SR(n,lambdaB_SR,lambdaR_SR,lambdaI_SR,lambdaU,lambdaD_max,muB_SR,muR_SR,muU(i));
  [A_MR_max(i),MTTF_MR_max,MTBF_MR_max(i)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaI_MR,lambdaU,lambdaD_MR_max,muB_MR,muR_MR,muU(i));
  [A_IR_max(i),MTTF_IR_max,MTBF_IR_max(i)] = evaluate_IR(nm,n,lambdaB_IR,lambdaR_IR,lambdaI_IR,lambdaU,lambdaD_max,muB_IR,muR_IR,muU(i));
  [A_CR_max(i),MTTF_CR_max,MTBF_CR_max(i)] = evaluate_CR(nm,n,lambdaB_CR,lambdaR_CR,lambdaI_CR,lambdaU,lambdaD_max,muB_CR,muR_CR,muU(i));
  %
...
csvwrite('results-AC-r17-varyingMuU.csv', [A_U_min A_SR_min MTBF_SR_min A_MR_min MTBF_MR_min A_IR_min MTBF_IR_min A_CR_min MTBF_CR_min A_U_max A_SR_max MTBF_SR_max A_MR_max MTBF_MR_max A_IR_max MTBF_IR_max A_CR_max MTBF_CR_max muU]);
