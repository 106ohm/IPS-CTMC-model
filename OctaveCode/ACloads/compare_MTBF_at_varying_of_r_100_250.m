...
for i = start:steps:stop
  ...
  lambdaR_MR = r(counter)*lambdaR_SR;
  lambdaI_MR = r(counter)*lambdaI_SR;
  muB_MR = muB_SR;% hours^(-1)
  muR_MR = muR_SR;

% Independent Redundancy (IR)
  lambdaB_IR = r(counter)*lambdaB_SR;
  lambdaR_IR = r(counter)*lambdaR_SR;
  lambdaI_IR = r(counter)*lambdaI_SR;
  muB_IR = muB_SR;% hours^(-1)
  muR_IR = muR_SR;

% Component Redundancy (CR)
  lambdaB_CR = r(counter)*lambdaB_SR;
  lambdaR_CR = r(counter)*lambdaR_SR;
  lambdaI_CR = r(counter)*lambdaI_SR;
  ...
  [A_MR(counter),MTTF_MR,MTBF_MR(counter)] = evaluate_MR(nm,n,lambdaB_MR,lambdaR_MR,lambdaI_MR,lambdaU,lambdaD,muB_MR,muR_MR,muU);
...
csvwrite('results-AC-r17-muU2-lambdaD0.125-varyingR-100-250.csv', [r I_SR I_IR I_CR]);
